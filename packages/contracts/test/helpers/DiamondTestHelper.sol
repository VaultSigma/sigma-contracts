...
contract DiamondTestHelper is IDiamondCut, IDiamondLoupe, Test {
     ...
     */
    function getSelectorsFromAbi(string memory abiPath) public view returns (bytes4[] memory) {
        ...
        for (uint256 i = 0; i < keys.length; i++) {
            bytes memory selector = vm.parseJsonBytes(abiJson, string.concat('$.methodIdentifiers.["', keys[i], '"]'));
